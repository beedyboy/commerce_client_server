...
createBuyerSettings: async ( buyer_id, created_at) => { 
  var reply = false; 
   await db('settings')
   .returning('id').insert({ buyer_id, created_at }).then( data => { 
     console.log({data});
  if (data.length > 0){
     reply = true;
     } else {reply = false; }
...
createSellerSettings: async ( id, created_at) => { 
  var reply = false; 
   await db('settings')
   .returning('id').insert({ shop_id: id, created_at }).then( data => { 
  if (data){
     reply = true;
     } else {reply = false; }
  }).catch(err => {
...
setSignature: async ( id, token_col, token) => {
  ...
  var reply = false;
   ...
   await db('logins').where('id', id).update(token_col, token).update('updated_at', updated_at).then( data => {
  if (data > 0){
     reply = true;
     }
  else {reply = false; }
